from utils import data, comunity_detection, mf_dcv, ic, fitness, rrset
...
def evaluate(individual, G, groups, ideal_influences, RR_SETS, communities):
    total_cov = 0
    total_rr = 0

    for cid, rr_sets in RR_SETS.items():
        nodes_in_comm = communities[cid]
        seeds_in_comm = [v for v in individual if v in nodes_in_comm]

        if len(rr_sets) > 0:
            cov = rrset.coverage(seeds_in_comm, rr_sets)
            total_cov += cov
            total_rr += len(rr_sets)

    # normalize influence
    influence = total_cov / total_rr if total_rr > 0 else 0
    mf, dcv = mf_dcv.calculate_MF_DCV(G, individual, groups, ideal_influences, p=PROPAGATION_PROB, mc=30)
...
def betterFIM(links_file, attr_file=None, attribute_name='region'):
    try:
    ...
    #Khởi tạo RR-set cho mỗi community 
    THETA = 1000
    ...
    for cid, nodes in communities.items():
        ...
        # tạo subgraph của community
        subgraph = G.subgraph(nodes).copy()

        # (OPTIONAL - khuyên dùng) thêm 1-hop neighbors để giảm mất cross-community influence
        for v in nodes:
            for u in G.neighbors(v):
                subgraph.add_edge(v, u)

        # generate RR-sets
        rr_sets = rrset.generate_rr_sets(
            subgraph,
            nodes,
            THETA,
            PROPAGATION_PROB
    ...
    for gen in range(MAX_GEN):
        ...
        for ind in population:
            ...
            result = evaluate(ind, G, groups, ideal_influences, RR_SETS, communities)
